...
from governance_recovery.cli import main  # noqa: E402


class TestGovernanceRecoveryCLI(unittest.TestCase):
    def test_cli(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Governance Recovery Plan", buf.getvalue())
            self.assertEqual(main(["--export", "--export-jsonl", "--base-dir", str(root)]), 0)
            self.assertTrue((root / ".control_plane" / "governance_recovery" / "latest.json").exists())
