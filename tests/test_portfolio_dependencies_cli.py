...
from portfolio_dependencies.cli import main  # noqa: E402


class TestPortfolioDependenciesCli(unittest.TestCase):
    def test_cli(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / ".config" / "portfolio" / "dependencies.json").write_text(
                json.dumps({"repositories": [{"repository_id": "A", "depends_on": [], "metadata": {}}]}),
                encoding="utf-8",
            )
            out = io.StringIO()
            rc = main(["--print", "--export", "--export-jsonl", "--base-dir", str(root)], stdout=out)
            self.assertEqual(rc, 0)
            self.assertIn("Portfolio Dependency Summary", out.getvalue())
            self.assertTrue((root / ".control_plane" / "portfolio_dependencies" / "latest.json").exists())
