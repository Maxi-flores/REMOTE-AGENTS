...
class TestPortfolioDriftStrategicIntegration(unittest.TestCase):
    def test_drift_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / ".control_plane" / "portfolio_drift" / "latest.json").write_text(
                json.dumps({"findings": [{"finding_id": "d1", "severity": "high", "repository_id": "A", "title": "Drift", "recommended_action": "Fix"}]}),
                encoding="utf-8",
            )
            ...
            self.assertTrue(any("Drift remediation" in title for title in titles))
