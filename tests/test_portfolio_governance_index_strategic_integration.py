...
class TestPortfolioGovernanceIndexStrategicIntegration(unittest.TestCase):
    def test_strategic_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / ".control_plane" / "portfolio_governance_index" / "latest.json").write_text(
                json.dumps({"report_id": "g1", "governance_score": 60, "governance_status": "degraded"}),
                encoding="utf-8",
            )
            ...
            self.assertTrue(any("Governance index improvement mission" in title for title in titles))
