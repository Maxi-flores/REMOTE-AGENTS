...
class TestPortfolioStrategicIntegration(unittest.TestCase):
    def test_portfolio_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / ".control_plane" / "portfolio" / "latest.json").write_text(
                json.dumps(
                    {
                        "report_id": "p1",
                        "recommended_execution_order": ["REMOTE-AGENTS", "Sentient-OS"],
            ...
            self.assertTrue(any("Portfolio priority" in t for t in titles))
