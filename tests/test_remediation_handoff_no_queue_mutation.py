...
class TestRemediationHandoffNoQueueMutation(unittest.TestCase):
    def test_queue_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plans = root / ".control_plane" / "remediation_plans"
            plans.mkdir(parents=True, exist_ok=True)
            (plans / "latest.json").write_text(json.dumps({"report_id": "r1", "batches": [], "items": []}), encoding="utf-8")
            rc = main(["--export", "--export-jsonl", "--base-dir", str(root)])
            self.assertEqual(rc, 0)
            after = qfile.read_text(encoding="utf-8")
